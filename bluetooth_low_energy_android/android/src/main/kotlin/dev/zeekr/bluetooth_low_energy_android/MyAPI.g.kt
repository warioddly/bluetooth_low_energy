  ...
  fun onStateChanged(stateArgsArg: MyBluetoothLowEnergyStateArgs, callback: (Result<Unit>) -> Unit)
{
  ...
  fun onDiscovered(peripheralArgsArg: MyPeripheralArgs, rssiArgsArg: Long, txPowerArgsArg: Long?, advertisementArgsArg: MyAdvertisementArgs, callback: (Result<Unit>) -> Unit)
{
    val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
    ...
    val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
    channel.send(listOf(peripheralArgsArg, rssiArgsArg, txPowerArgsArg, advertisementArgsArg)) {
      if (it is List<*>) {
        if (it.size > 1) {
          callback(Result.failure(FlutterError(it[0] as String, it[1] as String, it[2] as String?)))
        } else {
